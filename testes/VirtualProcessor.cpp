...
void VirtualProcessor::suspend_current_job_and_run_another() {
	...
	if (!current_job) {  // no jobs found locally
		...
		/* we got to make this to get some job from overloaded vps
		 * one way to do this is to use the context stack's size
		 * vps whose its stack's sizes are largest are overloaded
		*/
		for (it = vp_list.begin(); it != vp_list.end(); ++it) {
		 	if (*it != this) {
				current_job = (*it)->steal_job();
